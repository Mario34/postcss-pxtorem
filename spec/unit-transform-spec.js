...
describe("unitTransform", function() {
  it("should work on the readme example", function() {
    ...
    var processed = postcss(unitTransform()).process(input).css;

    expect(processed).toBe(output);
  });

  it("should replace the px unit with rem", function() {
    var processed = postcss(unitTransform()).process(basicCSS).css;
  ...
  it("should ignore non px properties", function() {
    ...
    var processed = postcss(unitTransform()).process(expected).css;
  ...
  it("should handle < 1 values and values without a leading 0 - legacy", function() {
    ...
    var processed = postcss(unitTransform(options)).process(rules).css;
...
describe("rootValue", function() {
  ...
  it("should replace using a root value of 10 - legacy", function() {
    ...
    var processed = postcss(unitTransform(options)).process(basicCSS).css;
  ...
  it("should replace using different root values with different files", function() {
    ...
    var processed1 = postcss(unitTransform(options)).process(basicCSS, {
      from: "/tmp/basic.css"
    }).css;
    var processed2 = postcss(unitTransform(options)).process(css2, {
...
describe("propWhiteList", function() {
  ...
  it("should only replace properties in the white list - legacy", function() {
    ...
    var processed = postcss(unitTransform(options)).process(css).css;
...
describe("mediaQuery", function() {
  ...
  it("should replace px in media queries", function() {
    ...
    var processed = postcss(unitTransform(options)).process(
...
describe("exclude", function() {
  it("should ignore file path with exclude RegEx", function() {
    ...
    var processed = postcss(unitTransform(options)).process(basicCSS, {
      from: "exclude/path"
    }).css;
